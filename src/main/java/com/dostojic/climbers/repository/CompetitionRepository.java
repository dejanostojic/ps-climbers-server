/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.dostojic.climbers.repository;

import com.dostojic.climbers.domain.Competition;
import com.dostojic.climbers.domain.valueobject.CompetitionSearchCriteria;
import java.util.List;

/**
 *
 * @author planina
 */
public interface CompetitionRepository {
    
    List<Competition> searchCompetitions(CompetitionSearchCriteria searchCriteria) throws Exception;

    Competition insert(Competition competition) throws Exception;
    
    Competition update(Competition competition) throws Exception;

    public Competition findById(Integer id) throws Exception;

}
